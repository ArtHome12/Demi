	...
	leftReagentAmounts(ourReaction->leftReagents.size())
{
	...
	// Предотвратим доступ со стороны потока, который может начать читать данную точку, до размещения и клеток и организма.
	// Скорей всего это состояние по-умолчанию, но на всякий случай.
	lockFlag.test_and_set(std::memory_order_acquire);

	// Разместим клетки и указатель на организм.
